  ...
  return (
    ...
    >
      <div className="flex flex-col justify-end text-white text-sm absolute inset-0 bg-black/50 p-10 rounded-2xl">
        <button
          onClick={prev}
          className="absolute top-30 left-5 text-white text-2xl bg-white/40 hover:bg-black/60 px-3 py-3 hover:cursor-pointer rounded-full"
        >
          <FaChevronLeft />
        </button>
        <Link to={`/moviedetails/${current.id}`}>
          <h3 className="text-xl hover:cursor-pointer">{current.title}</h3>
        </Link>
        <p>
        ...
        <button
          onClick={next}
          className="text-white text-2xl absolute right-5 top-30 bg-white/40 hover:bg-black/60 px-3 py-3 rounded-full hover:cursor-pointer"
        >
          <FaChevronRight />
        </button>
      </div>
